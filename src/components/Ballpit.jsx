...
class x {
  ...
  dispose() {
    ...
    // Avoid forceContextLoss: it can break a second WebGL context on the same tab
    // (e.g. React StrictMode remount) on some GPUs / drivers.
...
const Ballpit = ({ className = '', followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    ...
    const tryInit = () => {
      ...
      try {
        spheresInstance = createBallpit(canvas, { followCursor, ...props });
        spheresInstanceRef.current = spheresInstance;
      } catch (err) {
    ...
    return () => {
      ...
      if (spheresInstanceRef.current === spheresInstance) {
